...
public class NextFitWrapping
{
			...
			//	int cprint=0;
				for (int r=0; r < memory.size(); r++)
				{
					if(PreviousS==memory.size())
					{
						PreviousS=0; 
					}
					//System.out.println("inADD2"+PreviousS);
					if(memory.get(PreviousS).isHole())
					{
						//System.out.println("inADD3"+PreviousS);
						if(memory.get(PreviousS).getSize() >=process.size )
						{
							memory.add(PreviousS, process);
							++PreviousS;
			            	// Split the empty hole in half		            
			            		int sizeL =memory.get(PreviousS).getSize() - process.size;	
			            		memory.get(PreviousS).setSize(sizeL);
			            		if(sizeL==0) 
			            			{
			            				memory.remove(PreviousS);
			            			}			            	
							return true;
						}						
					}
